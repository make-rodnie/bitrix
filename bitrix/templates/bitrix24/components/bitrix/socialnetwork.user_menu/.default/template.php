	...
	<div class="profile-menu-inner">
		...
		<div class="profile-menu-right">
			<div class="profile-menu-info<?=$className?>">
				<a href="<?=$arResult["Urls"]["main"]?>" class="profile-menu-name"><?=$arResult["User"]["NAME_FORMATTED"]?></a><?if (array_key_exists("IS_ABSENT", $arResult) && $arResult["IS_ABSENT"]):?><span class="profile-menu-status"><?=GetMessage("SONET_UM_ABSENT")?></span><?endif;
				if (
					isset($arResult["User"]["ID"])
					&& (
			...
			<div id="profile-menu-filter" class="profile-menu-items"><?
				?><a href="<?=$arResult["Urls"]["Main"]?>" class="profile-menu-item<?if ($arParams["PAGE_ID"] == "user"):?> profile-menu-item-active<?endif?>"><?=GetMessage("SONET_UM_GENERAL")?></a><?
