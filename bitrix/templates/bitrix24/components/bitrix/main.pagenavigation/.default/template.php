...
if($arResult["REVERSED_PAGES"] === true)
{
	...
	do
	{
		...
		if ($arResult["START_PAGE"] <= 2 || $arResult["PAGE_COUNT"]-$arResult["START_PAGE"] <= 1 || abs($arResult['START_PAGE']-$arResult["CURRENT_PAGE"])<=2)
		{
			...
			elseif($arResult["START_PAGE"] == $arResult["PAGE_COUNT"]):
				?><a class="navigation-page-numb" href="<?=htmlspecialcharsbx($arResult["URL"])?>"><?=$NavRecordGroupPrint?></a><?
			else:
				?><a class="navigation-page-numb" href="<?=htmlspecialcharsbx($component->replaceUrlTemplate($arResult["START_PAGE"]))?>"><?=$NavRecordGroupPrint?></a><?
			endif;
			$bFirst = false;
			$bPoints = true;
		}
	...
	<div class="navigation-arrows">
		<<?if(!$bPrevDisabled):?>a href="<?=$sPrevHref;?>" id="<?=$ClientID?>_previous_page"<?else:?>span<?endif?> class="navigation-button<?if($bPrevDisabled):?> navigation-disabled<?endif?>"><span class="navigation-text"><span class="navigation-ctrl-before">Ctrl</span><span class="navigation-text-cont"><?=GetMessage("mp_nav_prev")?></span></span><?if(!$bPrevDisabled):?></a><?else:?></span><?endif?><<?if (!$bNextDisabled):?>a href="<?=$sNextHref;?>" id="<?=$ClientID?>_next_page"<?else:?>span<?endif?> class="navigation-button<?if($bNextDisabled):?> navigation-disabled<?endif?>"><span class="navigation-text"><span class="navigation-text-cont"><?=GetMessage("mp_nav_next")?></span><span class="navigation-ctrl-after">Ctrl</span></span><?if (!$bNextDisabled):?></a><?else:?></span><?endif?>
	</div>
	<?
}
else
	...
	do
	{
		if ($arResult["START_PAGE"] <= 2 || $arResult["END_PAGE"]-$arResult["START_PAGE"] <= 1 || abs($arResult['START_PAGE']-$arResult["CURRENT_PAGE"])<=2)
		{
			...
			elseif($arResult["START_PAGE"] == 1):
				?><a class="navigation-page-numb" href="<?=htmlspecialcharsbx($arResult["URL"])?>"><?=$arResult["START_PAGE"]?></a><?
			else:
				?><a class="navigation-page-numb" href="<?=htmlspecialcharsbx($component->replaceUrlTemplate($arResult["START_PAGE"]))?>"><?=$arResult["START_PAGE"]?></a><?
			endif;
			$bFirst = false;
			$bPoints = true;
		}
