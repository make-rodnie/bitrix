...
<div class="user-block" onclick="showUserMenu(this)">
	<span class="user-img user-default-avatar" <?if ($arResult["USER_PERSONAL_PHOTO_SRC"]):?>style="background: url('<?=$arResult["USER_PERSONAL_PHOTO_SRC"]?>') no-repeat center; background-size: cover;"<?endif?>></span><?if (!$arResult["SHOW_LICENSE_BUTTON"]):?><span class="user-name"><?=$arResult["USER_NAME"]?></span><?endif?>
</div>
