...
{
    $fullcount += $ar_res['ELEMENT_CNT'];
}
?>
                        <button type="button" class="btn btn-default"><strong><? echo $fullcount; ?>  </strong>items</button>
                        <div class="btn-group">
        ...
        while ($ar = $res->GetNext())
            {
                ...
                $db_res = CPrice::GetList(($b=""),
                          ($o=""),
                          Array("PRODUCT_ID"=> $idar));

if ($ar_res = $db_res->Fetch())
{
   $dbres = CCatalogProduct::GetList(($b=""),
                                   ($o=""),
                                   array("ID"=> $idar));
            if($fr = $dbres->Fetch())
                $quant = $fr['QUANTITY'];
